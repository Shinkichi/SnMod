...
simulated function UpdateHud()
{
	...
	if ( LAW(PawnOwner.Weapon) != none || Crossbow(PawnOwner.Weapon) != none
        || M79GrenadeLauncher(PawnOwner.Weapon) != none || GoldenM79GrenadeLauncher(PawnOwner.Weapon) != none
        || PipeBombExplosive(PawnOwner.Weapon) != none
        || HuskGun(PawnOwner.Weapon) != none || CrossBuzzSaw(PawnOwner.Weapon) != none
        || SPGrenadeLauncher(PawnOwner.Weapon) != none || M99SniperRifle(PawnOwner.Weapon) != none )
	{
		...
		else
		{
			...
			else if ( PawnOwner.Weapon.GetAmmoClass(0) != none )
			{
				...
				else if ( PipeBombExplosive(PawnOwner.Weapon) != none )
				{
					...
					|| GoldenM79GrenadeLauncher(PawnOwner.Weapon) != none)
				{
				...
				else if ( M99SniperRifle(PawnOwner.Weapon) != none )
				{
				...
				else
				{
					...
					if ( Flamethrower(PawnOwner.Weapon) != none || GoldenFlamethrower(PawnOwner.Weapon) != none )
					{
						DrawSpriteWidget(C, FlameIcon);
						DrawSpriteWidget(C, FlameTankIcon);
					}
					else if ( Shotgun(PawnOwner.Weapon) != none || BoomStick(PawnOwner.Weapon) != none
					 || Winchester(PawnOwner.Weapon) != none|| BenelliShotgun(PawnOwner.Weapon) != none
					 || Trenchgun(PawnOwner.Weapon) != none || CamoShotgun(PawnOwner.Weapon) != none 
					 || GoldenBenelliShotgun(PawnOwner.Weapon) != none )
					{
						DrawSpriteWidget(C, SingleBulletIcon);
						DrawSpriteWidget(C, BulletsInClipIcon);
					}
					else if ( ZEDGun(PawnOwner.Weapon) != none || ZEDMKIIWeapon(PawnOwner.Weapon) != none )
					{
						DrawSpriteWidget(C, ClipsIcon);
						DrawSpriteWidget(C, ZedAmmoIcon);
					}
					else if ( M32GrenadeLauncher(PawnOwner.Weapon) != none || CamoM32GrenadeLauncher(PawnOwner.Weapon) != none )
					{
						DrawSpriteWidget(C, M79Icon);
						DrawSpriteWidget(C, BulletsInClipIcon);
					}
